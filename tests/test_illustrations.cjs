...
(async()=>{
 const jobs=['generacion-v2.json','generacion-math.json','generacion-debate.json','generacion-historia.json'].flatMap(file=>JSON.parse(fs.readFileSync('assets/illustrations/'+file,'utf8')).images);
 assert.equal(jobs.length,27);let bytes=0;
 ...
 console.log('OK: '+jobs.length+' ilustraciones con canal alfa, fondo recortado y esquinas transparentes; '+Math.round(bytes/1024)+' KiB.');
