...
(async()=>{
 const jobs=JSON.parse(fs.readFileSync('assets/illustrations/generacion-v2.json','utf8')).images;
 assert.equal(jobs.length,11);let bytes=0;
 ...
 console.log('OK: 11 ilustraciones con canal alfa, fondo recortado y esquinas transparentes; '+Math.round(bytes/1024)+' KiB.');
