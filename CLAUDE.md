...
- El menú lateral es **una tabla de contenidos** (un marco, filas con reglas finas, número en columna propia), no una pila de tarjetas. La ruta activa se marca desde `data-route`; dentro de ella se despliegan sus partes y se resaltan con el desplazamiento, pero **sin mover el menú**: nunca `scrollIntoView` desde un oyente de scroll (eso lo congelaba al final de la página).
