...
- El menú lateral usa el **mismo lenguaje de celdas que `.art-list`**: fondo negro que asoma entre celdas a sangre, sin marco, radios ni sombra, y sin barra de desplazamiento propia (se desplaza `.side` entero). La ruta activa se marca desde `data-route`; dentro de ella se despliegan sus partes y se resaltan con el desplazamiento, pero **sin mover el menú**: nunca `scrollIntoView` desde un oyente de scroll (eso lo congelaba al final de la página).
