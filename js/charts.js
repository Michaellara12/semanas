...
(function(){
  ...
  function base(container,cfg){ container.innerHTML=""; const W=Math.max(280, container.clientWidth||600); const H=cfg.height||Math.max(220,Math.min(380,W*0.52)); const m=Object.assign({t:18,r:16,b:38,l:52},cfg.margin||{}); const svg=el("svg",{viewBox:`0 0 ${W} ${H}`,width:W,height:H,role:"img","aria-label":cfg.title||"gráfica"},container); return {svg,W,H,m,iw:W-m.l-m.r,ih:H-m.t-m.b,th:theme()}; }
  function axes(ctx,xs,ys,cfg){
    const {svg,m,iw,ih,th}=ctx; const g=el("g",{class:"axis"},svg);
    ys.ticks.forEach(t=>{ const y=ys.scale(t); el("line",{x1:m.l,x2:m.l+iw,y1:y,y2:y,class:"gridline",stroke:th.grid},g); const tx=el("text",{x:m.l-8,y:y+3,"text-anchor":"end",fill:th.text},g); tx.textContent=fmt(t,cfg.yFormat||"num"); });
    const n=xs.labels.length; const slot=xs.slot||iw/Math.max(1,n-1); const every=xs.slot?1:Math.max(1,Math.ceil(n/Math.max(3,Math.floor(iw/70)))); const maxChars=Math.max(6,Math.floor(slot/6.2));
    xs.labels.forEach((l,i)=>{ if(i%every!==0&&i!==n-1) return; const x=xs.scale(i); const tx=el("text",{x,y:m.t+ih+18,"text-anchor":"middle",fill:th.text},g); const s=String(l); if(s.length<=maxChars){ tx.textContent=s; return; }
      ...
      lines.forEach((ln,k)=>{ const ts=el("tspan",{x,dy:k===0?0:11},tx); ts.textContent=ln; }); });
  ...
  function line(container,cfg){
    ...
    (cfg.annotations||[]).forEach(a=>{ const x=xs.scale(a.i); el("line",{x1:x,x2:x,y1:m.t,y2:m.t+ih,class:"annot-line",stroke:th.pal[1]},svg); const t=el("text",{x:x+4,y:m.t+12,class:"annot",fill:th.text},svg); t.textContent=a.text; });
  ...
  function dual(container,cfg){
    const ctx=base(container,Object.assign({margin:{t:18,r:56,b:38,l:52}},cfg)); const {svg,m,iw,ih,th}=ctx; const labels=cfg.labels;
    ...
    ysR.ticks.forEach(t=>{ const y=ysR.scale(t); const tx=el("text",{x:m.l+iw+8,y:y+3,fill:R.color||th.pal[2],class:"lbl"},svg); tx.textContent=fmt(t,R.format); });
  ...
  function bar(container,cfg){
    ...
    cats.forEach((c,i)=>{ let accP=0,accN=0; series.forEach((s,si)=>{ const v=s.values[i]; if(v==null) return; const color=s.color||th.pal[si%th.pal.length]; const x=stacked?m.l+gw*i+(gw-inner)/2:m.l+gw*i+(gw-inner)/2+bw*si; let y1,y2; if(stacked){ if(v>=0){ y1=ys.scale(accP+v); y2=ys.scale(accP); accP+=v; } else { y1=ys.scale(accN); y2=ys.scale(accN+v); accN+=v; } } else { y1=Math.min(y0,ys.scale(v)); y2=Math.max(y0,ys.scale(v)); }
      ...
      if(cfg.valueLabels&&!stacked){ const t=el("text",{x:x+(bw-2)/2,y:y1-4,"text-anchor":"middle",class:"lbl",fill:th.text},svg); t.textContent=fmt(v,cfg.yFormat); } }); });
    container.classList.add("chart","animate");
  }
  ...
  function donut(container,cfg){ container.innerHTML=""; const th=theme(); const W=Math.max(220,container.clientWidth||300); const H=cfg.height||Math.min(280,W); const R=Math.min(W,H)/2-10; const r2=R*0.62; const svg=el("svg",{viewBox:`0 0 ${W} ${H}`,width:W,height:H},container); const cx=W/2,cy=H/2; const total=cfg.items.reduce((a,b)=>a+b.value,0); let a0=-Math.PI/2;
    cfg.items.forEach((it,i)=>{ const a1=a0+it.value/total*2*Math.PI; const large=a1-a0>Math.PI?1:0; const p=(a,rr)=>[cx+rr*Math.cos(a),cy+rr*Math.sin(a)]; const [x0,y0]=p(a0,R),[x1,y1]=p(a1,R),[x2,y2]=p(a1,r2),[x3,y3]=p(a0,r2); const d=`M${x0} ${y0} A${R} ${R} 0 ${large} 1 ${x1} ${y1} L${x2} ${y2} A${r2} ${r2} 0 ${large} 0 ${x3} ${y3} Z`; const path=el("path",{d,fill:it.color||th.pal[i%th.pal.length],class:"bar",stroke:th.bg,"stroke-width":2},svg); path.addEventListener("mousemove",ev=>showTip(`<b>${it.label}</b><br>${fmt(it.value,cfg.format||"pct")} (${(it.value/total*100).toFixed(1)} %)`,ev)); path.addEventListener("mouseleave",hideTip); a0=a1; });
    const t=el("text",{x:cx,y:cy-4,"text-anchor":"middle",class:"annot",fill:th.pal[3],style:"font-size:22px;font-weight:800"},svg); t.textContent=cfg.center||""; const t2=el("text",{x:cx,y:cy+16,"text-anchor":"middle",class:"lbl",fill:th.text},svg); t2.textContent=cfg.centerLabel||""; container.classList.add("chart"); }

  function pyramid(container,cfg){ const ctx=base(container,Object.assign({margin:{t:14,r:14,b:30,l:14}},cfg)); const {svg,m,iw,ih,th}=ctx; const g=cfg.groups; const maxv=Math.max(...g.map(x=>Math.max(x.m,x.f)))*1.05; const rowh=ih/g.length; const half=iw/2-28;
    g.forEach((x,i)=>{ const y=m.t+ih-(i+1)*rowh; const wm=x.m/maxv*half, wf=x.f/maxv*half; const rm=el("rect",{x:m.l+half-wm,y:y+1,width:wm,height:rowh-2,fill:cfg.colorM||th.pal[0],class:"bar",rx:2},svg); const rf=el("rect",{x:m.l+half+56,y:y+1,width:wf,height:rowh-2,fill:cfg.colorF||th.pal[1],class:"bar",rx:2},svg); [rm,rf].forEach(r=>{ r.style.animationName="none"; });
      ...
      if(i%2===0||g.length<=12){ const t=el("text",{x:m.l+half+28,y:y+rowh/2+3,"text-anchor":"middle",class:"lbl",fill:th.text},svg); t.textContent=x.label; } });
  ...
  function hist(container,cfg){ const vals=cfg.values.slice().sort((a,b)=>a-b); const n=vals.length; const bins=cfg.bins||30; const lo=cfg.min!=null?cfg.min:vals[0], hi=cfg.max!=null?cfg.max:vals[n-1]; const w=(hi-lo)/bins||1; const counts=new Array(bins).fill(0); vals.forEach(v=>{ let b=Math.floor((v-lo)/w); if(b>=bins) b=bins-1; if(b<0) b=0; counts[b]++; });
    const th=theme(); bar(container,{categories:counts.map((c,i)=>fmt(lo+w*i,cfg.xFormat||"num")),series:[{name:cfg.name||"frecuencia",values:counts.map(c=>c/n*100),color:cfg.color||th.pal[0]}],yFormat:"pct",yLabel:"% de simulaciones",height:cfg.height,margin:{l:46}});
    if(cfg.markers){ const svg=container.querySelector("svg"); const W=+svg.getAttribute("width"); const m={l:46,r:16}; const iw=W-m.l-m.r; const H=+svg.getAttribute("height"); cfg.markers.forEach(mk=>{ const x=m.l+(mk.value-lo)/(hi-lo)*iw; if(x<m.l||x>m.l+iw) return; el("line",{x1:x,x2:x,y1:18,y2:H-38,class:"annot-line",stroke:th.pal[1]},svg); const t=el("text",{x:x+4,y:30,class:"annot",fill:th.text},svg); t.textContent=mk.text; }); } }
